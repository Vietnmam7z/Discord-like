...
def login(username, password, login_window, response_label):
    ...
    try:
        ...
        threading.Thread(target=receive_response, args=(client_socket, login_window, response_label), daemon=True).start()

    except socket.error as e:
        print(f"Lỗi kết nối: {e}")

def receive_response(client_socket, login_window, response_label):
...
def open_login_window():
    ...
    entry_password = tk.Entry(login_window, width=30, fg="grey", show="*")
